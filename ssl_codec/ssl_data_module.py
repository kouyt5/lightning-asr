...
class SSLAudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.ssl_folder is not None and self.on_the_flying is False:
            fp = open(data["ssl_audio_filepath"], 'rb')
            wav2vec_feature = pickle.load(fp)  # np.array 1*T*512 
            fp.close()
            return wav2vec_feature, text2id, data['audio_filepath'], self.mask
        return data["audio_filepath"], text2id, data['audio_filepath'], self.mask
...
class SSLDataModule(pl.LightningDataModule):
    ...
    def _collate_fn(self, batch):
        ...
        if self.on_the_flying and not self.ssl_model_retrain:  # 如果自监督模型不需要重新训练
            with torch.no_grad():
                inputs, input_percentages = self.ssl_model([item[0] for item in batch])
                inputs = inputs.unsqueeze(1)  # B*1*T*512
        ...
        if not self.on_the_flying:
            # 如果使用文件夹加载特征
            # 构建输入向量
            max_seq_len = max(batch, key=lambda x:x[0].shape[1])[0].shape[1]
            feature_dim = max(batch, key=lambda x:x[0].shape[1])[0].shape[2]
            inputs = torch.zeros(minibatch_size, 1, max_seq_len, feature_dim)
            input_percentages = torch.FloatTensor(minibatch_size)  # mask 使用
            for i in range(len(batch)):
                inputs[i].narrow(1, 0,batch[i][0].shape[1]).copy_(torch.as_tensor(data=batch[i][0]))
                input_percentages[i] = batch[i][0].shape[1] / float(max_seq_len)
        inputs = inputs.transpose(2,3)  # B*1*E*T
        inputs = inputs.cpu()
        if batch[0][3]:  # 是否mask，测试集不mask
            inputs = self.features_cutout(inputs)
